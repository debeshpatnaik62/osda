 ...
 * ========================================================= */
 ...
 (function($) {
	...
	Calendar.prototype = {
		...
		_initializeOptions: function(opt) {
			...
			this.options = {
				...
				style: opt.style == 'background' || opt.style == 'border' || opt.style == 'custom' ? opt.style : 'background',
		...
		_initializeDatasourceColors: function() {
			for(var i in this.options.dataSource) {
				if(this.options.dataSource[i].holidaytype==1){
					this.options.dataSource[i].color = colors[0];
				}
				if(this.options.dataSource[i].holidaytype==2){
					this.options.dataSource[i].color = colors[1];
				}
			...
			switch(this.options.style)
			{
				case 'border':
					...
					if(events.length == 1) {
						...
						elt.parent().css('background-color', 'rgb(6, 68, 105)');
					}
	...
	var colors = $.fn.calendar.colors = ['#064469', '#D2AC2B'];
