...
<script language="javascript">
       ...
       // $('#btnSaveMainMenu').hide();
        ...
        <?php 
         foreach($viewGl as $row2)
          {
       ?>
                fillMenuList(<?php echo $row2['pageId'];?>, <?php echo $row2['tinMenuType'];?>, 'primaryLink', 'GLMenu<?php echo $row2['pageId'].'_'.$row2['tinMenuType'];?>')
                $("#GLMenu<?php echo $row2['pageId'].'_'.$row2['tinMenuType'];?>").sortable({revert: true});
        <?php
          }
        ?>
        if ('<?php echo $outMsg != '' ?>')
	...
	var glIds		= $('#selGlobalMenu').val();		
        ...
        var errflag 	= 0;
        var totalCount	= 0;
         var selmenuType     = $("#selGlobalMenu option:selected").data("menu");
        for (var i = 0; i < arrCount; i++)
        {
            if ($("#chkPageId" + idArrs[i]).is(':checked'))
            {
                flag++;
                $(".mainMenuClass"+glIds+"_"+selmenuType).each(function () {
                    totalCount++;
                    var hdnVal          = $(this).val();
                    ...
                    if(hdnVal == checkedVal && menuType==selmenuType)
                    {
                        errflag = 1;
                    }
                });
                ...
                if (errflag > 0)
                {
                    ...
                    var hdnFld = '<input type="hidden" name="mainMenuArr'+glIds+'_'+selmenuType+'[]" data-menu="'+selmenuType+'" id="hdnMainMenuId' + idArrs[i] + '" class="mainMenuClass'+glIds+'_'+selmenuType+'" value="' + idArrs[i] + '" />';
                    var menuItem = '<div class="ui-sortable-handle mainMenuItem" id="mainMenuItem' + idArrs[i] + '">' + menuText + hdnFld + closeBtn + '</div>';
                    $("#GLMenu"+glIds+"_"+selmenuType).append(menuItem);
    ...
    {       
	 var selmenuType                = $("#selGlobalMenu option:selected").data("menu");
		...
		$(".glPortlet:checked").each(function () {
                    var glId	= $(this).val();
                //console.log(glId+"_"+selmenuType)    
                ...
                <input type="hidden" name="hdnFldForPageId" id="hdnFldForPageId" value="" />
                    ...
                    <div class="portletCategoryClass">
                        <div class="innerPortletPl">
                            <h4>
                                <input type="checkbox" class="glPortlet" name="chkGLId[]" id="chkGLId<?php echo $row['pageId'].'_'.$row['tinMenuType'];?>" value="<?php echo $row['pageId'].'_'.$row['tinMenuType'];?>" /><?php echo $row['menuName'].$strExtmenu;?>
                                
                            </h4>
                            <div class="scrollable">
                                <div id="GLMenu<?php echo $row['pageId'].'_'.$row['tinMenuType'];?>" class="ui-sortable"></div>
                            </div>
                        </div>                        
                    </div>   
                    <?php 
