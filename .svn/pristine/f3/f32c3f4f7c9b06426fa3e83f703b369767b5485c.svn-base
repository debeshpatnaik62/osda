...
//============ Button Submit ===================
        ...
        { 
            $applicantId        = (!empty($_POST['hdn_ids']))?$_POST['hdn_ids']:0;
            $result             =  $objPanel->addUpdateMarks($applicantId);
            $outMsgMark         =  $result;            
            $redirectLoc        =  APP_URL."enterMark/".$glId."/".$plId;
        }  
...
//=================== View skill records ================================
        ...
        if ($isPaging==0) {
            $result   = $objPanel->managePanel('PGR',$intRecno,0,$skillId,'','','','','','','','',$distId,0,$userId,$intPgSize,'','',0,0);
        } else {
            $intPgno            = 1;
            $intRecno           = 0;
            $result   = $objPanel->managePanel('VR',0,0,$skillId,'','','','','','','','',$distId,0,$userId,0,'','',0,0);
        }

        $totalResult            = $objPanel->managePanel('CR',0,0,$skillId,'','','','','','','','',$distId,0,$userId,0,'','',0,0);
