<?php
	...
	$strTab             =($id>0)?'Edit':'SAMS ITI Institute Tag';
   ...
   //========== Permission ===============  
    ...
    $instituteName      =  $objInstitute->fillInstituteITIOSDA('1');
    $instituteNameArray = $instituteName['instituteArray'];

    $samsInstituteArray  =  $objInstitute->fillSamsITIInstitute('1');
...
//print_r($samsInstituteName);
    ...
    $resultArray=array();
                    if (mysqli_num_rows($result) > 0) 
                    {
                       ...
                       {
                           $samsid                                         = $row['intSAMSITIInstituteId'];
                           $resultArray[$samsid]['osdaVal']                = $row['intOSDAInstituteId'];
                           $ctr++; 
                           //echo "<pre>";print_r($samsid."hii".$resultArray[$samsid]['osdaVal']);echo "</pre>";
                       }
