<?php
	...
	$strTab             =($id>0)?'Edit':'Add';
        $strclick           =($id>0)?"window.location.href='". APP_URL."viewDepartment/".$glId."/".$plId."';":"";
	//========== Default variable ===============	
	$flag               = 0;  
        $errFlag            = 0;
	$outMsg             = '';	
	...
	if(isset($_POST['btnSubmit']))
	{
           ...
           $strDescription  = htmlspecialchars_decode($result['strDescription'],ENT_QUOTES);
             ...
             if($flag==0 && $id>0)
	          $redirectLoc	=  APP_URL."viewDepartment/".$glId."/".$plId."/".$id;  
	     else if($flag==0 && $id==0)  
	          $redirectLoc	=  APP_URL."viewDepartment/".$glId."/".$plId;
