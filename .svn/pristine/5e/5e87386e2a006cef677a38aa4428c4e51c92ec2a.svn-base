<?php
    ...
    Functions Used	: manageDesignation,deleteDesignation
    ==================================================*/
    ...
    $explPriv       = $objDesg->checkPrivilege($userId, $glId, $plId, $pageName, 'V');
	...
	if(isset($_REQUEST['hdn_qs'])&& $_REQUEST['hdn_qs']!='' )
	{
		...
		$outMsg	= $objDesg->deleteDesignation($qs,$ids);
	}
        if($isPaging==0)	
            $result		= $objDesg->manageDesignation('PG',$intRecno,$intLocation,$intDepartment,'','',0,$userId);	
	else                                    
	{
            $intPgno	= 1;
            $intRecno	= 0;
            $result                 = $objDesg->manageDesignation('V',0,$intLocation,$intDepartment,'','',0,$userId);
	}
       $totalResult                 = $objDesg->manageDesignation('V',0,$intLocation,$intDepartment,'','',0,$userId);
