<?php
 /* ================================================
	  File Name         	  : viewNanoUnicornDetailsInner.php
...
//============ Permission =====================//
     ...
     $explPriv   = $objNano->checkPrivilege($userId, $glId, $plId, $pageName, 'V');
...
//============ Search Function ==============================//
 ...
 //============= Delete Records ================================	
        if(isset($_REQUEST['hdn_qs'])&& $_REQUEST['hdn_qs']!='' )
        {
                ...
                $outMsg   	= $objNano->deleteNanoUnicorn($qs,$ids);
        }		

//=================== View feedBack records ================================
        if ($isPaging==0) {
            $result		= $objNano->manageNanoUnicorn('PG',$intRecno,$strName,0,'','',$intPgSize,0,0,$strStartDt,$strEndDate,0,'0000-00-00',0);
        } else {
            $intPgno	= 1;
            $intRecno	= 0;
            $result		= $objNano->manageNanoUnicorn('V',0,$strName,0,'','',0,0,0,$strStartDt,$strEndDate,0,'0000-00-00',0);
        }

        $totalResult	= $objNano->manageNanoUnicorn('CT',0,$strName,0,'','',0,0,0,$strStartDt,$strEndDate,0,'0000-00-00',0);
