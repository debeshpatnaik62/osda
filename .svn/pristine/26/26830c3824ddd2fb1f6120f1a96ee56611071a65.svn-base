  ...
  //print_r($_SESSION);exit;
    ...
    if($isPaging==0 && $_REQUEST['hdn_PageNo']=='' && $_REQUEST['ID']>0)
    {
        ...
        if($isPaging==0){
            ...
            $arrConditions=array('reportID'=>$intRecno ,'pageCount'=>20,'insId'=>$id);

            $result     = $objIns->manageSkillInsRegd('PGP', $arrConditions); 
    }
    ...
    else
    {
            ...
            $intRecno   = 0;
            $arrConditions=array('reportID'=>$intRecno ,'pageCount'=>20,'insId'=>$id);
            $result                 = $objIns->manageSkillInsRegd('VP',$arrConditions);
          //  print_r($result);exit;
    }
           $intPgSize                   = 20;

       $totalResult                 = $objIns->manageSkillInsRegd('VP',$arrConditions);
