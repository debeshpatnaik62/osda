<?php
    /* ================================================
    File Name         	: archiveInstituteInner.php
    Description		: This page is used to view archieved institute details.
	...
	if($isPaging==0 && $_REQUEST['hdn_PageNo']=='' && $_REQUEST['ID']>0)
	{
		...
		unset($_SESSION['paging']);
         ...
         $intType    = (isset($_REQUEST['selType']) && $_REQUEST['selType'] != '')?$_REQUEST['selType']:'0';
        ...
        if(isset($_REQUEST['btnSearch']))
        {
            $intPgno        = 1;
            $intRecno       = 0;
            $intDistid      = $_REQUEST['ddlDistrict'];
            $intType        = $_REQUEST['selType'];
            $strInstName    = trim($_REQUEST['txtInstName']);
        }
        ...
        try{
         //============= Delete/Active/Inactive function =================
            ...
            if($isPaging==0)	
                $result                 = $objInstitute->manageInstitute('PG',$intRecno,$intDistid,$strInstName,'',$intType,0,'','','','','','','','','','',0,1,0,$intPgSize,'','',0,'',0,0,0,'');
            else                                    
            {
                $intPgno                = 1;
                $intRecno               = 0;
                $result                 = $objInstitute->manageInstitute('V',0,$intDistid,$strInstName,'',$intType,0,'','','','','','','','','','',0,1,0,$intPgSize,'','',0,'',0,0,0,'');
            }
            $totalResult                = $objInstitute->manageInstitute('V',0,$intDistid,$strInstName,'',$intType,0,'','','','','','','','','','',0,1,0,$intPgSize,'','',0,'',0,0,0,'');
