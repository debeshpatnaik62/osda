<?php
 /* ================================================
    ...
    Description           : This is used for qualified skill development candidate
...
//=================== View skill records ================================
        ...
        if ($isPaging==0) {
            $arrConditions=array('reportID'=>$intRecno , 'pageCount'=>20,'instituteName' =>$instituteName,'tpId'=>$soughtFor,'marks'=>$marks,'qualify'=>$qualify,'instituteId'=>$instituteId);
            $result     = $objDevelop->manageskillDevelopment('PGQ', $arrConditions); 
        } else {
            $intPgno            = 1;
            $intRecno           = 0;
            $arrConditions=array('reportID'=>$intRecno , 'pageCount'=>20,'instituteName' =>$instituteName,'tpId'=>$soughtFor,'marks'=>$marks,'qualify'=>$qualify,'instituteId'=>$instituteId);
            $result     = $objDevelop->manageskillDevelopment('VQ', $arrConditions); 
        }

       
       $totalResult                 = $objDevelop->manageskillDevelopment('VQ',$arrConditions);
