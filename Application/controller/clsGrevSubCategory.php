...
// Function To Add Upadate Grev Category By::Rahul Kumar Saw   :: On:: 03-Apr-2023 
    public function addUpdateGrevCategory($catId) {
      ...
      if($newSessionId == $hdnPrevSessionId) {   
        ...
        $txtCategory1           = htmlspecialchars(addslashes($_POST['selectCategory']), ENT_QUOTES, 'UTF-8');
        $errCategory1           = Model::isSpclChar($_POST['selectCategory']);

        $txtCategory           = htmlspecialchars(addslashes($_POST['txtSubCategory']), ENT_QUOTES);
        $blankCategory         = Model::isBlank($txtSubCategory);
        $errCategory           = Model::isSpclChar($_POST['txtSubCategory']);
        $lenCategory           = Model::chkLength('max', $txtSubCategory,200);
        
        $txtCategory_O           = htmlspecialchars($_POST['txtSubCategory_O'], ENT_QUOTES, 'UTF-8');
        $errCategoryO            = Model::isSpclChar($txtSubCategory_O);
        ...
        else if(($errCategory>0) || ($errCategory1>0) || ($errDescription>0 || $errDescriptionO>0 || $errCategoryO>0))
        {
                $errFlag		= 1;
