<?php
	...
	$strTab         = ($id>0)?'Edit':'Add';
        $strclick       = ($id>0)?"window.location.href='". APP_URL."viewUser/".$glId."/".$plId."';":"$('#userImage').hide();";
        //========== Permission ===============	
        $glId           = $_REQUEST['GL'];
        $plId          = $_REQUEST['PL'];
	...
	if(isset($_POST['btnSubmit']))
	{
           ...
           $outMsg        =  $result['msg']; 
           $flag          =  $result['flag'];          
            $strFullname     =  $result['strFullname'];                   
            $intLocId        =  $result['intLocId'];
            $intDeptId       =  $result['intDeptId']; 
            $intDesigId      =  $result['intDesigId']; 
            $intGender       =  $result['intGender']; 
            $strQualification      =  $result['strQualification'];
            $maxSL         =  $result['intSlno']; 
            $strPhno         =  $result['strPhno'];
            $intMobileNo        =  $result['strMobno'];
            ...
            $intPrivilege     =  $result['intPrevilage'];
                ...
                if($flag==0 && $id>0)
	          $redirectLoc	=  APP_URL."viewUser/".$glId."/".$plId."/".$id;  
	       else if($flag==0 && $id==0)  
	          $redirectLoc	=  APP_URL."viewUser/".$glId."/".$plId;
